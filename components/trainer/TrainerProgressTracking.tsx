...
interface TrainerProgressTrackingProps {
  traineeId: string
  trainerId: string
}

export function ProgressTracking({ traineeId, trainerId }: TrainerProgressTrackingProps) {
  const t = useTranslations('trainer.progressTracking')
  ...
  useEffect(() => {
    fetch(`/api/programs?trainerId=${trainerId}&traineeId=${traineeId}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  const fetchBodyWeight = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/body-weight?traineeId=${traineeId}&weeks=${weeks}`)
  ...
  return (
    <div className="space-y-6">
      ...
      <Card title={tp('title')}>
        <ProgressPhotoGallery traineeId={traineeId} readOnly />
      </Card>
